...
const char *http_request_line(int fd, char *reqpath, size_t req_len, char *env, size_t *env_len)
{
    ...
    *sp2 = '\0';
    ...
    url_decode(reqpath, sp1, req_len);
...
void dir_join(char *dst, size_t dstlen, const char *dirname, const char *filename) {
    strncpy(dst, dirname, dstlen);
    if (dst[strlen(dst) - 1] != '/')
        strncat(dst, "/", dstlen - strlen(dirname) - 1);
    strncat(dst, filename, dstlen - strlen(dst));
}
...
void http_serve_directory(int fd, const char *pn) {
    ...
    for (i = 0; indices[i]; i++) {
        dir_join(name, 1024,  pn, indices[i]);
        if (stat(name, &st) == 0 && S_ISREG(st.st_mode)) {
            dir_join(name, 1024, getenv("SCRIPT_NAME"), indices[i]);
            break;
        }
    }
...
//if theres no \0 in the first len bytes the dest will not be null terminated
{
    for (char *stop = dst + len; dst <= stop; )
    {
